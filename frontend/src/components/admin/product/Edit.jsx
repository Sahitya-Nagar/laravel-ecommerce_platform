...
//             } else {
//                 //This below method will fetch the errors to display on the user side from the API side
...
//                                           }
//                                           type="number" placeholder='Compare Price' className='form-control' />
//                                         </div>
//                                       </div>
...
//                                                 <button type='button' className='btn btn-secondary mt-3 w-100 mb-3' onClick={() => changeImage(productImage.image)}>Set as Default</button>
//                                               </div>
//                                             )
//                                           })
//                                         }
...
//                                 <button 
//                                 disabled={disable}
//                                 type='submit' className='btn btn-primary mt-3 mb-3'>Update</button>
//                         </form>
//                     </div>
...
const Edit = ({ placeholder }) => {
  const editor = useRef(null);
  const [content, setContent] = useState(''); // State for JoditEditor content
  const [disable, setDisable] = useState(false);
  const [categories, setCategories] = useState([]);
  const [brands, setBrands] = useState([]);
  const [sizes, setSizes] = useState([]);
  ...
  const {
    ...
    defaultValues: async () => {
      ...
      // Update product images and sizes
      setProductImages(result.data.product_images);
      setSizesChecked(result.productSizes);
      // Update JoditEditor content with fetched description
                      ...
                      {productImages &&
                        productImages.map((productImage, index) => (
                          <div className="col-md-3" key={`image-${index}`}>
                            ...
                            <button
                              type="button"
                              className="btn btn-secondary mt-3 w-100 mb-3"
